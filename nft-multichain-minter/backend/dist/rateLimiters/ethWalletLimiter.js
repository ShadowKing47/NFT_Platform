...
exports.ethWalletLimiter = ethWalletLimiter;
 ...
 */
async function ethWalletLimiter(req, res, next) {
    try {
        ...
        if (count >= MAX_REQUESTS_PER_WALLET) {
            logger_1.default.warn(`Ethereum rate limit exceeded for wallet: ${walletAddress}`);
            res.status(429).json({
        ...
        logger_1.default.error({ err: error }, "Ethereum wallet rate limiter error");
        next(); // Fail open on error
    }
}
exports.default = ethWalletLimiter;
//# sourceMappingURL=ethWalletLimiter.js.map