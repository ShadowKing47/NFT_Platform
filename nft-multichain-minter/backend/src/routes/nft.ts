    ...
    if (nftInfo.metadata.length > 0) {
      try {
        ...
        if (metadataString.startsWith("ipfs://") || metadataString.startsWith("Qm") || metadataString.startsWith("bafy")) {
          ...
          try {
            const response = await axios.get(ipfsGateway, { timeout: 5000 });
        ...
        if (metadata.attributes && Array.isArray(metadata.attributes)) {
          properties = metadata.attributes.map((attr: any) => ({
...
router.get("/ethereum/:tokenId", async (req, res, next) => {
  try {
    ...
    const provider = new ethers.JsonRpcProvider(
      process.env.ETH_RPC_URL || "https://sepolia.infura.io/v3/YOUR_INFURA_KEY"
    );
